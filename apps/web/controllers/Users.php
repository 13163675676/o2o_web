 ...
 * @Last Modified time: 2017-12-18 09:57:17
 */
...
class Users extends \CLASSES\WebBase
{
    ...
    public function Login()
    {
        ...
        {
            $this->exportData('手机号或验证码不能为空');
        }

        /*获取验证码信息*/
        $dao_verify_code = new \WDAO\Verifies();
        ...
        {
            $this->exportData('验证码不正确或验证码已过有效期');
        }

        /*获取用户信息*/
        $dao_users = new \WDAO\Users();
        ...
        if(!empty($user_data['data']['0']['u_id'])){
            ...
            if($res){
                ...
                $this->exportData(array('token'=>$token,'u_img'=>$u_img,'u_online'=>$user_data['data']['0']['u_online'],'u_name'=>$user_data['data']['0']['u_true_name'],'u_sex'=>$user_data['data']['0']['u_sex'],'u_id'=>$user_data['data']['0']['u_id'],'u_pass'=>$user_data['data']['0']['u_pass'],'u_idcard'=>$u_idcard ));
            }
            ...
            if ($u_id)
            {
                ...
                $this->exportData(array('token'=>$token,'u_img'=>$this ->web_config['u_img_url'].'0'.'.jpg','u_online'=>'0','u_name'=>$data['u_name'],'u_sex'=>'-1','u_id'=>"$u_id",'u_pass'=>'','u_idcard'=>''));
            }else{
                $this->exportData('注册失败,请重新注册');
            }
        }
    ...
    public function sendVerifyCode($msg='用户您好,您的登录验证码为',$number='')
    {
        ...
        if(!empty($phone_number))
        {
            ...
            if($res){
                $this->exportData('短信发送成功');
            }else{
                $this->exportData('短信发送失败');
            }

        }else{
            $this->exportData('请填写手机号码');
        }
    }
    ...
    public function favorateTasks()
    {
        ...
        if(empty($u_id)){
            $this->exportData('请输入用户id');
        }

        $dao_favorate = new \WDAO\Users_favorate();
    ...
    public function favorateUsers()
    {
        ...
        if(empty($favorate_arr['data'])){
            $this->exportData( array('data'=>array()));
        }
        foreach ($favorate_arr['data'] as $key => $value) {
        ...
        $dao_users_ext_info = new \WDAO\Users_ext_info();
        $users_ext_arr = $dao_users_ext_info -> listData(array(
            'u_id' => array('type' => 'in', 'value' => $favorate_id_arr),
            'fields'=>'u_id,uei_info','pager'=>false,
            )
        );
        $dao_users_cur_position = new \WDAO\Users_cur_position();
        ...
        /*获取分类数组*/
        $this->exportData( array('data'=>$res));
    }
    ...
    public function usersInfo()
    {
        ...
        /*用户地区详情*/
        $dao_ext_info = new \WDAO\Users_ext_info();
    ...
    public function usersInfoEdit()
    {
        ...
        /*判断传入数据长度*/
        if(isset($data_users['u_true_name']) && mb_strlen($data_users['u_true_name'],'utf8') > 25) $this->exportData('昵称长度最长为25个');
        if(isset($data_users['u_idcard']) && mb_strlen($data_users['u_idcard'],'utf8') > 19) $this->exportData('身份证号长度最长为19个');
        if(isset($data_ext['uei_info']) && mb_strlen($data_ext['uei_info'],'utf8') > 250) $this->exportData('个人简介的最大字符长度为250');
        if(isset($data_ext['uei_address']) && mb_strlen($data_ext['uei_address'],'utf8') > 80) $this->exportData('详细地址信息的最大字符长度为80');
        ...
        if(isset($data_ext['uei_zip']) && strlen($data_ext['uei_zip']) > 6) $this->exportData('邮编的最大字符长度为6');



        if(!empty($data_ext)){
            $dao_users_ext = new \WDAO\Users_ext_info();
        ...
        if($res > 0){
            $this->exportData('用户信息修改失败');
        }else{
            $this->exportData('用户信息修改成功');
        }
    ...
    /*users_cur_posit*/
        ...
        if(!empty($list['data'])){
            ...
            if (isset($_REQUEST['fu_id']) && intval($_REQUEST['fu_id']) > 0) {
                ...
                /*获取收藏列表*/
                $dao_users_favorate = new \WDAO\Users_favorate();
    ...
    public function getUsersFundsLog()
    {
        if(empty($_GET['u_id']) || empty($u_id = intval($_GET['u_id']))){
            $this->exportData('用户id不能为空');
        }
        $category = !empty($_GET['category'])  ? trim($_GET['category']) : 'all';
        $dao_platform_funds_log = new \WDAO\Platform_funds_log();
        $r_data = array();
        $w_data = array();
        /*充值*/
        ...
        $dao_recharge_log = new \WDAO\User_recharge_log();
        ...
        $dao_withdraw_log = new \WDAO\User_withdraw_log();
...
/*支出收入订单合并*/
        ...
        $this->exportData( array('data'=>$data));
        ...
        if(!empty($content)){
            if (file_put_contents($this ->web_config['u_img_path'].$filename, $content))
            {
                ...
                \Swoole\Image::thumbnail($this ->web_config['u_img_path'].$filename,
                            ...
                            1000);
                unlink($this ->web_config['u_img_path'].$filename);
                $this->exportData('头像修改成功');

            }else{
                $this->exportData('头像写入失败');
            }
        }else{

            $this->exportData('您没有上传图片');
        }
    ...
    public function msgDel()
    {
        ...
        if(empty($_GET['um_id']) || empty($um_id = intval($_GET['um_id']))){
            $this->exportData('用户站内信关系ID为空');
        }

        $dao_user_msg = new \WDAO\User_msg();
        $res = $dao_user_msg ->updateData(array(
            'um_status' => '-1',
            ),array('um_id'=>$um_id));
        if($res){
            $this->exportData('信息删除成功');
        }else{
            $this->exportData('信息删除失败');
        }
    ...
    public function msgReadEdit()
    {
        ...
        if($res){
            $this->exportData('状态修改成功');
        }else{
            $this->exportData('状态修改失败');
        }
    }
    ...
    {
        if(empty($_GET['u_id']) || empty($u_id = intval($_GET['u_id']))){
            $this->exportData('用户ID为空');
        }
        if(empty($_GET['ucp_posit_x']) || empty($ucp_posit_x = floatval($_GET['ucp_posit_x']))){
            $this->exportData('用户x轴信息为空');
        }
        if(empty($_GET['ucp_posit_y']) || empty($ucp_posit_y = floatval($_GET['ucp_posit_y']))){
            $this->exportData('用户y轴信息为空');
        }
        ...
        $dao_cur_position = new \WDAO\Users_cur_position();
        ...
        if($res){
            $this->exportData('位置信息修改成功');
        }else{
            $this->exportData('位置信息修改失败');
        }
    }
    ...
    public function complaintsAdd()
    {
        ...
        if(empty($_POST['c_id']) || empty(intval($_POST['c_id']))){
            ...
            $data['ct_id'] = $ct_id;
            $data['c_in_time'] = time();
            $dao_complaints = new \WDAO\Complaints();

            $c_id = 0;
            $c_id = $dao_complaints ->addData($data);

            if($c_id <= 0) {
                $this->exportData('投诉信息写入失败');
            }
            ...
            if(!empty($c_id)){
                ...
                $dao_complaints_ext = new \WDAO\Complaints_ext();
                $res_ext_add = $dao_complaints_ext -> addData($ext_data);
                if($res_ext_add){
                    $this->exportData(array('投诉信息写入成功','c_id'=>$c_id));
                }
            ...
            if(isset($_POST['c_id']) && !empty(intval($_POST['c_id'])) && !empty($_POST['c_img'])){
                ...
                if(!empty($img_path) && intval($res) >= 0){
                    ...
                    if($res){
                        $this->exportData('图片信息修改成功');
                    }else{
                        $this->exportData('图片信息修改失败');
                    }
                }else{
                    $this->exportData('图片信息写入失败');
                }

            }
        }
        $this->exportData('参数不足,图片信息写入失败!');
    }
    ...
    public function applyWithdraw()
    {
        ...
        if(isset($this ->web_config['withdraw_amount_min']) && $this ->web_config['withdraw_amount_min'] > 0)
        {
            ...
            {
                $this->exportData('提现失败!最小提现金额为'.$this ->web_config['withdraw_amount_min'].'元;');
            }
        }
        if(isset($this ->web_config['withdraw_amount_max']) && $this ->web_config['withdraw_amount_max'] > 0)
        {
            ...
            {
                $this->exportData('提现失败!最大提现金额为'.$this ->web_config['withdraw_amount_max'].'元;');
            }
        }
        ...
        if(!$check_res){
            $this->exportData('提现密码错误');
        }
        $dao_user_withdraw_log = new \WDAO\User_withdraw_log();
        $dao_users_ext_funds = new \WDAO\Users_ext_funds();
        /*判断用户余额是否大于提现余额*/
        ...
        if(!empty($uef_overage['uef_overage'])){
            if(intval($uef_overage['uef_overage']) >= $uwl_amount){
                ...
                $this->exportData('提现申请添加成功');

                /*处理提现过程end*/
            }else{
                $this->exportData('用户余额不足');
            }
        }else{
            $this->exportData('用户余额不足');
        }
        ...
        public function applyRechargeLog()
    {
        ...
        if(empty($_REQUEST['url_amount']) || floatval($_REQUEST['url_amount']) <= 0){
            $this->exportData('充值金额必须大于0元');
        }else{
            $url_amount = floatval($_REQUEST['url_amount']);
        }
        if(empty($_REQUEST['p_id']) || empty($p_id = intval($_REQUEST['p_id']))){
            $this->exportData('支付方式不能为空');
        }
        ...
        if(isset($this ->web_config['recharge_amount_min']) && $this ->web_config['recharge_amount_min'] > 0)
        {
            ...
            {
                $this->exportData('充值失败!最小充值金额为'.$this ->web_config['recharge_amount_min'].'元;');
            }
        }
        if(isset($this ->web_config['recharge_amount_max']) && $this ->web_config['recharge_amount_max'] > 0)
        {
            ...
            {
                $this->exportData('充值失败!最大充值金额为'.$this ->web_config['recharge_amount_max'].'元;');
            }
        }
        ...
        if (!empty(intval($RC_log_id))) {
            ...
            if ($p_id = $this ->web_config['wx_pid'])/*微信支付*/
            {
                ...
                if($res){
                    ...
                    $this->exportData('充值失败!系统错误请联系管理员');
                }
            }
    ...
    public function commentAdd()
    {
        ...
        $dao_task_comment = new \WDAO\Task_comment();
        ...
        if(!empty($data_ext) && intval($tc_id) > 0) {
            ...
            $dao_task_comment_ext = new \WDAO\Task_comment_ext();
            $dao_task_comment_ext -> addData($data_ext);
        }
        if(intval($tc_id) > 0){
           $this->exportData( array('data'=>array('tc_id'=>$tc_id)));
        }else{
           $this->exportData("评价失败");
        }
    }
    /**********************************************************添加好评次数**********************************************************/
    /*评价修改接口*/
    ...
    public function passwordEdit()
    {
        ...
        elseif(isset($_REQUEST['u_mobile']) && !empty($num = intval($_REQUEST['u_mobile'])) && !empty($_REQUEST['u_idcard']))
        {
            ...
            $u_info = $dao_users ->listData(array('u_mobile'=>$num,'u_idcard'=>$u_idcard,'fields'=>'u_id','pager'=>false));
            if (!empty($u_info['data'][0]['u_id']) && intval($u_info['data'][0]['u_id']) > 0) {
                    $this ->exportData('身份证号验证成功');
                }else{
                    $this ->exportData('身份证号验证失败');
                }
        ...
        elseif(isset($_REQUEST['u_id']) && !empty($u_id = intval($_REQUEST['u_id'])) && !empty($_REQUEST['u_pass']))
        {
            $check_user_res = $dao_users ->checkUserPayPassword(array('u_id' => $u_id,'u_pass' => trim($_REQUEST['u_pass'])));
            if(isset($check_user_res['u_id']) && !empty(intval($check_user_res['u_id']))){
                $this ->exportData('原密码正确');
            }else{
                $this ->exportData('原密码错误');
            }
        }
        ...
        {
            $this ->exportData('参数不足,密码修改失败');
        }
        if($res){
            $this ->exportData('密码修改成功');
        }else{
            $this ->exportData('密码修改失败,请联系管理员');
        }
    }
